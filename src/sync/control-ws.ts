...
export class ControlChannel {
	...
	on<T extends ControlMessageType>(
		type: T,
		handler: (msg: ControlMessageMap[T]) => void,
	): void {
	...
	off<T extends ControlMessageType>(
		type: T,
		handler: (msg: ControlMessageMap[T]) => void,
	): void {
	...
	private async encryptAndSend(msg: ControlMessage): Promise<void> {
		if (!this.e2e || !this.ws || this.ws.readyState !== WebSocket.OPEN)
			return;
		try {
			if (
				(msg.type === "file-chunk-start" ||
				...
				const encryptedPath =
					typeof msg.path === "string"
						? await this.e2e.encryptString(msg.path)
						: msg.path;
				this.ws.send(
					JSON.stringify({
						...msg,
				...
				if (op && typeof op.content === "string") {
					...
					if (typeof op.path === "string")
						encryptedOp.path = await this.e2e.encryptString(
							op.path,
						);
					if (typeof op.oldPath === "string")
						encryptedOp.oldPath = await this.e2e.encryptString(
							op.oldPath,
						);
					if (typeof op.newPath === "string")
						encryptedOp.newPath = await this.e2e.encryptString(
							op.newPath,
						);
					this.ws.send(
						JSON.stringify({
	...
	private async decryptAndDispatch(
		raw: ControlMessage & { encrypted?: boolean },
	): Promise<void> {
		if (!this.e2e) return;
		try {
			...
			if (
				...
				const decryptedPath =
					typeof msg.path === "string"
						? await this.e2e.decryptString(msg.path)
						: msg.path;
				decryptedMsg = {
					...msg,
					data: decryptedData,
				...
				if (op && typeof op.content === "string") {
					...
					if (typeof op.path === "string")
						decryptedOp.path = await this.e2e.decryptString(
							op.path,
						);
					if (typeof op.oldPath === "string")
						decryptedOp.oldPath = await this.e2e.decryptString(
							op.oldPath,
						);
					if (typeof op.newPath === "string")
						decryptedOp.newPath = await this.e2e.decryptString(
							op.newPath,
						);
					decryptedMsg = {
						...msg,
